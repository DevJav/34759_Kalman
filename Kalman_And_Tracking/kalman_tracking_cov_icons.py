...
def recolor_icon(icon_path, color):
    ...
    int_tuple_color = tuple([int(c * 255) for c in tuple_color])
    overlay = Image.new("RGBA", icon.size, int_tuple_color + (255,) )  # Create a solid color overlay
    recolored = Image.composite(overlay, icon, a)  # Apply color to non-transparent parts

    return recolored

def get_icon(object_type, color):
    import os
    path = 'icons/'
    path = os.path.join(os.path.dirname(__file__), path)

    image = recolor_icon(path + object_type + '.png', color)
    return image

def get_labels():
    labels_path = '34759_final_project_rect/seq_02/'
